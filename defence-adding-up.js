...
// @description   Skrypt sumuje wsparcie z nadchodzących własnych komend i komend współplemieńców. Modyfikacja sumująca łączny potencjał defensywny.
// @author        stivens, Makak, ABC*
...
(function () {
        ...
        gui_content =
            `<table class="vis" style="width:100%">
							...
							<tr id="script_table">
								...
								<td><input type="text" id="filter_date" size="5" value="` +
            filter_data.date +
            `"></td>
								<td><input type="text" id="filter_hour"size="8" value="` +
            filter_data.time +
            `"></td>
							</tr>
						</table>
    ...
    function refresh_gui() {
        ...
        var all_troops = sum_troops_objects(
            sum_troops_objects(own_commands_troops, ally_commands_troops),
            in_village_troops
        );
    ...
    function create_units_table(units) {
        ...
        for (var unit in units) {
            if (units.hasOwnProperty(unit)) {
                ...
                incoming_own +=
                    '<td style="text-align:center"><span id="commands_script_own_' + unit + '">' + 0 + "</span></td>";
                incoming_allies +=
                    '<td style="text-align:center"><span id="commands_script_ally_' + unit + '">' + 0 + "</span></td>";
                in_village +=
                    '<td style="text-align:center"><span id="commands_script_in_village_' + unit + '">' + 0 + "</span></td>";
                all += '<td style="text-align:center"><span id="commands_script_all_' + unit + '">' + 0 + "</span></td>";
            }
        }

        header +=
            '<th style="text-align:center"><img src="https://dspl.innogamescdn.com/asset/6052b745/graphic//buildings/barracks.png"></th>';
        incoming_own += '<td style="text-align:center"><span id="commands_script_own_sum' + '">' + 0 + "</span></td>";
        incoming_allies += '<td style="text-align:center"><span id="commands_script_ally_sum' + '">' + 0 + "</span></td>";
        in_village += '<td style="text-align:center"><span id="commands_script_in_village_sum' + '">' + 0 + "</span></td>";
        all += '<td style="text-align:center"><span id="commands_script_all_sum' + '">' + 0 + "</span></td>";

        return (
            `<table style="width:100%">
					<tr style="height: 25px"><th>Typ wojsk</th>` +
            header +
            `</tr>
					<tr><td>Własne komendy</td>` +
            incoming_own +
            `</tr>
					<tr><td>Cudze komendy</td>` +
            incoming_allies +
            `</tr>
					<tr><td>W wiosce</td>` +
            in_village +
    ...
    function update_units_table(own_commands, allies_commands, in_village, all) {
        ...
        $("#commands_script_own_sum").text(
            (own_commands["spear"] + own_commands["sword"] + own_commands["light"] + own_commands["heavy"] * 4) || 0
        );
        $("#commands_script_ally_sum").text(
            (allies_commands["spear"] + allies_commands["sword"] + allies_commands["light"] + allies_commands["heavy"] * 4) || 0
        );
        $("#commands_script_in_village_sum").text(
            (in_village["spear"] + in_village["sword"] + in_village["light"] + in_village["heavy"] * 4) || 0
        );
        $("#commands_script_all_sum").text(
            (all["spear"] + all["sword"] + all["light"] + all["heavy"] * 4) || 0
        );
    }
    ...
    function toggle_load_button() {
        ...
        if (total_commands_count === unknown_commands_count && total_commands_count > 0) {
            $("#script_table_header")
                .html('<button type="button" style="width:100%">Kliknij aby wczytać komendy</button>')
                .css("text-align", "center");
        } else if (unknown_commands_count > 0) {
            $("#script_table_header")
                .html(
                    '<button type="button" style="width:100%">Kliknij aby wczytać ' +
                    get_unknown_commands_count(timestamp, true, true) +
                    " nieznane komendy</button>"
                )
                .css("text-align", "center");
        } else if (unknown_commands_count === 0) {
            gui_content = "Wczytano wszystkie komendy<br>" + gui_content;
            $("#script_table_header")
                .html('<span style="text-align-center; width:100%;">Wczytano wszystkie komendy</span>')
                .css("text-align", "center");
        }
    }
    ...
    function generate_links_to_filter() {
        if (get_commands_count(-1, true, true) !== 0) {
            var button =
                '<td><span class="small command_script_filterbutton" style="cursor: pointer; display: block; width: 16px; height: 22px; overflow: hidden; background: transparent url(' +
                image_base +
                '/index/group-jump.png) no-repeat;"><img src="' +
                image_base +
                '/blank-16x22.png" alt="Wprowadź czas tej komendy do filtra" title="Wprowadź czas tej komendy do filtra"></span></td>';
            ...
            $(".command_script_filterbutton").on("click", function () {
                ...
                $("#filter_hour").val(
                    format_number(date_parts.hour) +
                    ":" +
                    format_number(date_parts.minute) +
                    ":" +
                    format_number(date_parts.second)
                );
    ...
    function get_filter_timestamp() {
        if ($("#commands_script_filter").is(":checked")) {
            var input_date = $("#filter_date")
                .val()
                .trim()
                .match(/^(\d{1,2})\.(\d{1,2})$/);
            var input_time = $("#filter_hour")
                .val()
                .trim()
            ...
            if (input_date === null || input_time === null) {
                ...
                UI.ErrorMessage(
                    "Podano nieprawidłową datę lub czas (wpisano np. 70 w liczbę minut lub inny tego typu błąd)",
                    5000
                );
            } else {
                var timestamp = date_parts_to_timestamp({
                    day: input_date[1],
                    month: input_date[2],
                    hour: input_time[1],
                    minute: input_time[2],
    ...
    function troops_in_info_village() {
        var troops = {};
        var elements = $('form#withdraw_selected_units_village_info table tr');
    ...
    function get_simulator_link(def_units, wall_level) {
        var link =
            "/game.php?screen=place&mode=sim&simulate&att_axe=6200&att_light=3100&att_ram=400&luck=0&def_wall=" + wall_level;
    ...
    function get_commands_ids(timestamp, get_own_commands_ids, get_ally_commands_ids) {
        ...
        $(commands_selector).each(function () {
            ...
            if (
                (span.hasClass("commandicon-ally") === false && get_own_commands_ids) ||
                (span.hasClass("commandicon-ally") && get_ally_commands_ids)
            ) {
    ...
    function download_command_data(command_id) {
        $.get(
            ...
            .fail(function () {
                UI.ErrorMessage(
                    "Wystąpił błąd podczas pobierania danych o komendach. Odśwież stronę aby spróbować ponownie.",
                    2000
                );
                doing_ajax = false;
            });
    ...
    function update_commands_data(timestamp) {
        var commands_ids = get_unknown_commands_ids(get_commands_ids(timestamp, true, true)),
            i = 0,
            n = commands_ids.length;
        if (n > 0) {
